...
public class PriceRuleServiceImpl implements PriceRuleService{
    @Override
    public double getRecommendedPrice(List<ProductDetail> productDetailList, ProductMarket productMarket) {
        // 1) calculate average price for product
        Supplier<Stream<ProductDetail>> streamSupplier = ()->productDetailList.stream().filter(e -> e.getCode().equals(productMarket.getName()));
        double averagePrice = streamSupplier.get().mapToDouble(value -> value.getPrice()).average().getAsDouble();
        // 2) filter out promotion price
        Stream<ProductDetail> filterPromotionStream = streamSupplier.get().filter(e -> e.getPrice() >= averagePrice * 0.5);
        // 3) filter out error price
        Stream<ProductDetail> filterErrorStream = filterPromotionStream.filter(e -> e.getPrice() <= averagePrice * 1.5);
        ...
        Map<Double, Long> collect = doublePriceList.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));//.forEach((k, v) -> {
        // finally return the price with most occurrence
